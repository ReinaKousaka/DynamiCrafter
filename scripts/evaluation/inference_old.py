...
def get_epic_data(data_dir='/workspace/DynamiCrafter/Epic', video_id='P35_101',
    ...
    def get_list_by_stride(start, stride):
        return list(range(start, start + stride * num_frames, stride))
    stride = random.choice([2, 3, 5])
    frame_indices = get_list_by_stride(start_frame, stride)
...
def run_inference(args, gpu_num, gpu_no):
    ...
    for line in epic_meta_file:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            for idx in range(2):
                videos = data_info['pixel_values']
                prompts = data_info['text'] + ',' + narration

                if idx == 0:
                    camera_data_source = data_info
                    name_flag = 'original'
                else:
                    if last_data_info is None: continue
                    camera_data_source = last_data_info
                    name_flag = 'modified'
                print(f'prompts {prompts}')
                batch_samples = image_guided_synthesis(
                    model, prompts, videos, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, \
                    args.unconditional_guidance_scale, args.cfg_img, 30 // data_info['stride'], args.text_input, args.multiple_cond_cfg, args.loop, args.interp, args.timestep_spacing, args.guidance_rescale,
                    plucker_embedding = camera_data_source['plucker_embedding'],
                    extrinsics = camera_data_source['extrinsics'],
                    intrinsics = camera_data_source['intrinsics'],
                    camera_embeddings = camera_data_source['camera_embeddings'],
                    epipolar_masks = camera_data_source['epipolar_masks'],
                )
