...
def project_rays(
    ...
    origins = homogenize_points(origins).float()
    origins = einsum(world_to_cam, origins, "... i j, ... j -> ... i")
    directions = homogenize_vectors(directions).float()
    ...
    result = {
        "t_min": torch.empty_like(projection_at_zero["t"]).float(),
        "t_max": torch.empty_like(projection_at_infinity["t"]).float(),
        "xy_min": torch.empty_like(projection_at_zero["xy"]).float(),
        "xy_max": torch.empty_like(projection_at_infinity["xy"]).float(),
        "overlaps_image": torch.empty_like(projection_at_zero["valid"]).float(),
    }
    ...
    for min_valid, max_valid in itertools.product([True, False], [True, False]):
        ...
        result["t_min"][mask] = min_value["t"][mask].float()
        result["t_max"][mask] = max_value["t"][mask].float()
        result["xy_min"][mask] = min_value["xy"][mask].float()
        result["xy_max"][mask] = max_value["xy"][mask].float()
        result["overlaps_image"][mask] = (min_value["valid"] & max_value["valid"])[mask].float()
